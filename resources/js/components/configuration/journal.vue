 <template>
    <div>
        <div class="row">
            <div v-for="client in clientCurrent">
                <div class="col cursor-pointer">
                    <div class="mb-4  rounded-lg p-2" :class="[client.id === currentID ? 'bg-primary' : 'bg-light-blue disabled-div']">
                        <div class="rounded-lg p-3 bg-white"
                             role="button"
                             @click="selectClient(client)">
                            <div class="h1 text-primary font-weight-bold"> <img :src="'/images/logo/'+client['cllogo']" style="height: 80px" class="bg-white inline-block" alt="User Image"></div>
                            <span class="text-primary d-block text-uppercase font-weight-bold h6 m-0">{{ client['clnmcl'] }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <template v-if="clientSelected">
            <hr/>
        <div class="row">
            <div class="col cursor-pointer d-none" :class="connexion == 0 ? 'disabled-div' : ''">
...
export default { 
    ...
    methods: {
        getActivity(page=1){
            ...
            axios.get('/activity/getInfos/'+this.clientCurrentSelect.slug+"?page="+page+"&paginate="+this.paginate+"&type="+this.typeSelected).then(response => {
        ...
        getCount(type) {
            axios.get(`/activity/count/${type}`+'?client_id='+this.clientCurrentSelect.id)
                .then(response => {
        ...
        selectClient(client){
            this.clientCurrentSelect = client; 
